  ...
  bool PandaPositionController::init(panda_hardware_interface::SharedJointInterface* hw, ros::NodeHandle &n){
    if(!panda_sim_controllers::JointArrayController<panda_effort_controllers::JointPositionController>::init(hw, n)) {
      ...
      if (n.getParam("topic_joint_command", topic_name)) {
        ros::NodeHandle nh("~");
        //ROS_WARN_STREAM("panda postion controller's namespace is:~~~~~~~~~~~~~~:" << nh.getNamespace().c_str());
        //ROS_WARN_STREAM("position controller getparam's namespace is:~~~~~~~~: " << n.getNamespace().c_str());
      ...
      std::string topic_joint_controller_states;
      if (n.getParam("topic_joint_controller_states",topic_joint_controller_states))
      {
        //ROS_WARN_STREAM("position controller getparam's namespace is:~~~~~~~~: " << n.getNamespace().c_str());
        controller_states_publisher_.reset(
        new realtime_tools::RealtimePublisher<franka_core_msgs::JointControllerStates>(n, topic_joint_controller_states, 1));
      }
      else
      {
        controller_states_publisher_.reset(
