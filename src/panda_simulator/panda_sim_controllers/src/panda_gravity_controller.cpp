...
namespace panda_sim_controllers {
  ...
  bool PandaGravityController::init(panda_hardware_interface::SharedJointInterface* hw, ros::NodeHandle &n){
    ...
    if (n.getParam("topic_robot_state", topic_name)) {
        ros::NodeHandle nh("~");
        // ROS_WARN_STREAM("graverty controller getparam's namespace is:~~~~~~~~: " << n.getNamespace().c_str());
        sub_joint_command_ = nh.subscribe(topic_name, 1, &PandaGravityController::gravityCommandCB, this);
      } else {
