...
PandaGripperController::~PandaGripperController() {
  ...
  state_pub_.shutdown();
  delete grasp_action_server;
  delete move_action_server;
  delete stop_action_server;
  delete homing_action_server;
}
...
bool PandaGripperController::init(panda_hardware_interface::SharedJointInterface* robot,
  ...
  // Store nodehandle
  nh_ = nh;
  std::string homing_topic;
  if(nh_.getParam("homing_topic",homing_topic))
  {
    homing_action_server = new actionlib::SimpleActionServer<franka_gripper::HomingAction>(nh_, homing_topic, boost::bind(&PandaGripperController::homing, this, _1), false);
  }else{
    homing_action_server = new actionlib::SimpleActionServer<franka_gripper::HomingAction>(nh_, "/franka_gripper/homing", boost::bind(&PandaGripperController::homing, this, _1), false);
  }
  std::string stop_topic;
  if(nh_.getParam("stop_topic",stop_topic))
  {
    stop_action_server = new actionlib::SimpleActionServer<franka_gripper::StopAction>(nh_, stop_topic, boost::bind(&PandaGripperController::stop, this, _1), false);
  }else{
    stop_action_server = new actionlib::SimpleActionServer<franka_gripper::StopAction>(nh_, "/franka_gripper/stop", boost::bind(&PandaGripperController::stop, this, _1), false);
  }
  std::string move_topic;
  if(nh_.getParam("move_topic",move_topic))
  {
    move_action_server = new actionlib::SimpleActionServer<franka_gripper::MoveAction>(nh_, move_topic, boost::bind(&PandaGripperController::move, this, _1), false);
  }else{
    move_action_server = new actionlib::SimpleActionServer<franka_gripper::MoveAction>(nh_, "/franka_gripper/move", boost::bind(&PandaGripperController::move, this, _1), false);
  }
  std::string grasp_topic;
  if(nh_.getParam("grasp_topic",grasp_topic))
  {
    grasp_action_server = new actionlib::SimpleActionServer<franka_gripper::GraspAction>(nh_, grasp_topic, boost::bind(&PandaGripperController::grasp, this, _1), false);
  }else{
    grasp_action_server = new actionlib::SimpleActionServer<franka_gripper::GraspAction>(nh_, "/franka_gripper/grasp", boost::bind(&PandaGripperController::grasp, this, _1), false);
  }
  ...
  for (const auto joint_it : xml_struct) {
    ...
    // Get the joint-namespace nodehandle
    {
      ...
      gripper_controllers_[i].reset(new panda_effort_controllers::JointPositionController());
  ...
  // ROS_WARN_STREAM("nh_ in gripper controller is:~~~~~~~~~~~~~~~" << nh_.getNamespace().c_str());
  ...
  {// They provided a custom topic to subscribe to
    ...
    state_pub_ = nh_base.advertise<sensor_msgs::JointState>(joint_state_topic, 1, true);
  }
  else
  {
